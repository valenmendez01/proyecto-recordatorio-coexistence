...
export function EventSheet({ event, open, onOpenChange }: EventSheetProps) {
  // mutate global de SWR — recibe cualquier llave del caché
  ...
  // Llave SWR de la semana actualmente visible
  const startDate = format(currentWeekStart, "yyyy-MM-dd");
  const endDate = format(endOfWeek(currentWeekStart, { weekStartsOn: 1 }), "yyyy-MM-dd");
  const swrKey = ['reservas-semana', startDate, endDate] as const;
  ...
  const handleSaveChanges = async () => {
    ...
    if (result.success) {
      // ── Mutación optimista ────────────────────────────────────────────────
      // Actualizamos el caché local de forma inmediata sin esperar Realtime.
      // revalidate: false porque Realtime disparará un UPDATE poco después y
      // aplicará el mismo patch nuevamente (idempotente, sin parpadeo).
      // Si la fecha cambió a otra semana, el evento simplemente deja de
      // aparecer en los días visibles — comportamiento correcto.
      mutate(
        swrKey,
  ...
  const updateStatus = async (status: "confirmado" | "cancelado" | "reservado") => {
    ...
    if (result.success) {
      ...
      // Patch optimista: actualiza solo el campo estado sin un fetch extra.
      // Realtime llegará pronto y confirmará el cambio (revalidate: false evita
      // que se dispare un fetch al mismo tiempo que Realtime está procesando).
      mutate(
        swrKey,
        (cache: CalendarEvent[] | undefined) => {
          if (!cache) return cache;
  ...
  const confirmDeleteEvent = async () => {
    ...
    if (result.success) {
      ...
      // Eliminación inmediata del caché — el evento desaparece del calendario
      // sin esperar la revalidación. Realtime disparará un DELETE poco después
      // que intentará filtrar el mismo id (ya no existe → no-op).
      mutate(
        swrKey,
        (cache: CalendarEvent[] | undefined) => {
          if (!cache) return cache;
          return cache.filter((e) => e.id !== event.id);
        },
        { revalidate: false }
      );
